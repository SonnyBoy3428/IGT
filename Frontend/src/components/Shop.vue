<template>
  <div class="container-fluid shop">
    <h1>Shop</h1>
    <div class="row">
      <div class="item col-sm-3" v-for="item in items2" :key="item">
        ...
        <div class="card-body">
          ...
          <a href="#" class="btn btn-primary" @click.prevent="log({ name: item.name, price: item.price })">Jetzt kaufen</a>
        </div>
      </div>
    </div>
  </div>
    ...
    computed: {
      items2: function() {
        ...
        for (var i = 0; i < 50; i++) {
          arr.push({
            name: `Artikel ${i+1}`,
            price: ((Math.random() + 0.1) * 70).toFixed(2)
          });
        }
        return arr;
      }
